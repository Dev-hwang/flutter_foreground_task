...
class ForegroundService : Service(), MethodChannel.MethodCallHandler {
	...
	private fun initBackgroundChannel() {
        ...
        this.bundlePath = currFlutterLoader?.findAppBundlePath()
    ...
    private fun buildButtonCompatActions(): List<NotificationCompat.Action> {
		...
		for (i in buttons.indices) {
			...
			val bTextColor = buttons[i].textColorRgb?.let(::getRgbColor)
			val bText = getTextSpan(buttons[i].text, bTextColor)
			val bicon = buttons[i].iconData?.let(::getIconResId) ?: 0
			val bAction = NotificationCompat.Action.Builder(bicon, bText, bPendingIntent).build()
			actions.add(bAction)
		}
	...
	private fun getIconResIdFromIconData(iconData: IconData): Int {
		return getIconResId(iconData) ?: 0
	}
	...
	private fun getIconResId(iconData: IconData): Int? {
		...
		return if (iconResType.isEmpty()
			...
			getIconResId(iconResType, iconResPrefix, iconName)
		}
	}

	private fun getIconResId(resType: String, resPrefix: String, name: String): Int {
